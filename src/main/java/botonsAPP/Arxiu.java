...
public class Arxiu {
    ...
    public void display(PApplet processing, float x, float y, float w, float h){
        ...
        processing.fill(200, 100, 100); processing.stroke(0); processing.strokeWeight(3);
        ...
        for(int r = 0; r < numRows; r++){
            ...
            for(int c = 0; c< numCols; c++){
                if(r==0){
                    processing.text(tableHeaders[c], xCol + 10, y + (r+1)*rowHeight - 10);
                }
                else{
        ...
        processing.fill(0);
        processing.text("Pag: "+(this.numPage+1)+" / "+(this.numTotalPages+1), x, y + h + 50);
